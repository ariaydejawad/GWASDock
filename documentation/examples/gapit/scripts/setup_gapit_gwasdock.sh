...
# Relative path to the GAPIT tool directory within the repository
GAPIT_TOOL_DIR="./tools/gapit"

# Docker image name
DOCKER_IMAGE_NAME="gapit-container:dev"
...
LOCAL_INPUT_DIR="/Users/dong/Project/GWASDock/gapit-resource/input"  
LOCAL_OUTPUT_DIR="/Users/dong/Project/GWASDock/gapit-resource/output" 
LOCAL_SCRIPTS_DIR="/Users/dong/Project/GWASDock/gapit-resource/scripts" 

# Data directories inside the container (expected paths by the gapit program)
CONTAINER_INPUT_DIR="/app/workspace/gapit/input"
CONTAINER_OUTPUT_DIR="/app/workspace/gapit/output"
CONTAINER_SCRIPTS_DIR="/app/workspace/gapit/user-scripts"
...
log_info "--- GWASDock GAPIT Setup Script ---"
...
# Step 4. Build GAPIT Docker Image
log_info "Navigating to GAPIT tool directory: $GAPIT_TOOL_DIR"
cd "$GAPIT_TOOL_DIR" || log_error "Failed to change directory to $GAPIT_TOOL_DIR. Make sure it exists."

log_info "Building Docker image: $DOCKER_IMAGE_NAME"
docker buildx build \
  --iidfile gapit_container_iid.txt \
...
if [ ! -d "$LOCAL_SCRIPTS_DIR" ]; then
    log_info "Local scripts directory '$LOCAL_SCRIPTS_DIR' does not exist. Creating it."
    mkdir -p "$LOCAL_OUTPUT_DIR" || log_error "Failed to create local scripts directory."
fi
# Step7.  Run the Container 
log_info "Running GAPIT container with bind mounts..."
...
log_info "Mapping local output: $LOCAL_SCRIPTS_DIR -> Container output: $CONTAINER_SCRIPTS_DIR"
...
docker container run -it --rm \
  ...
  --mount type=bind,source="$LOCAL_SCRIPTS_DIR",target="$CONTAINER_SCRIPTS_DIR" \
