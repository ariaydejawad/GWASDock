...
# Relative path to the GCTA tool directory within the repository
GCTA_TOOL_DIR="./tools/gcta"

# Docker image name
DOCKER_IMAGE_NAME="gcta-container:dev"
...
LOCAL_INPUT_DIR="/Users/dong/Project/GWASDock/gcta-resource/input"  
LOCAL_OUTPUT_DIR="/Users/dong/Project/GWASDock/gcta-resource/output" 

# Data directories inside the container (expected paths by the GCTA program)
CONTAINER_INPUT_DIR="/workspace/program/gcta/input"
CONTAINER_OUTPUT_DIR="/workspace/program/gcta/output"
...
log_info "--- GWASDock GCTA Setup Script ---"
...
# Step 4. Build GCTA Docker Image
log_info "Navigating to GCTA tool directory: $GCTA_TOOL_DIR"
cd "$GCTA_TOOL_DIR" || log_error "Failed to change directory to $GCTA_TOOL_DIR. Make sure it exists."
...
ARCH=$(uname -m)
if [ "$ARCH" = "x86_64" ]; then
    # For x86_64 architecture, use the standard build
    log_info "Detected architecture: x86_64"
    docker buildx build \
    --file tools/gcta/Dockerfile.amd64 \
    --platform=linux/amd64 \
    --iidfile gcta_iid.txt \
    --tag "$DOCKER_IMAGE_NAME" . || log_error "Failed to build Docker image."
elif [ "$ARCH" = "arm64" ]; then
    # For ARM64 architecture, use the build with --platform linux/arm64
    log_info "Detected architecture: ARM64 (aarch64)"     
    docker buildx build \
    ...
    log_error "Unsupported architecture: $ARCH. This script currently supports x86_64 and ARM64 architectures only."
fi
...
log_info "Running GCTA container with bind mounts..."
