...
# Relative path to the TASSEL tool directory within the repository
TASSEL_TOOL_DIR="./tools/tassel"

# Docker image name
DOCKER_IMAGE_NAME="tassel-container:dev"
...
LOCAL_INPUT_DIR="/Users/dong/Project/GWASDock/tassel-resource/input"  
LOCAL_OUTPUT_DIR="/Users/dong/Project/GWASDock/tassel-resource/output" 

# Data directories inside the container (expected paths by the TASSEL program)
CONTAINER_INPUT_DIR="/workspace/program/tassel/input"
CONTAINER_OUTPUT_DIR="/workspace/program/tassel/output"
...
log_info "--- GWASDock TASSEL Setup Script ---"
...
# Step 4. Build TASSEL Docker Image
log_info "Navigating to TASSEL tool directory: $TASSEL_TOOL_DIR"
cd "$TASSEL_TOOL_DIR" || log_error "Failed to change directory to $TASSEL_TOOL_DIR. Make sure it exists."

log_info "Building Docker image: $DOCKER_IMAGE_NAME"
docker buildx build \
  --iidfile tassel_container_iid.txt \
...
log_info "Running TASSEL container with bind mounts..."
